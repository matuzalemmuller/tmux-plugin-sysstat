...
print_mem() {
  ...
  local mem_free=$(echo $mem_usage | /usr/bin/original-awk -v scale="$size_scale" '{ print $1/scale }')
  local mem_used=$(echo $mem_usage | /usr/bin/original-awk -v scale="$size_scale" '{ print $2/scale }')
  ...
  mem_view="${mem_view//'#{mem.color}'/$(echo "$mem_color" | /usr/bin/original-awk '{ print $1 }')}"
  mem_view="${mem_view//'#{mem.color2}'/$(echo "$mem_color" | /usr/bin/original-awk '{ print $2 }')}"
  mem_view="${mem_view//'#{mem.color3}'/$(echo "$mem_color" | /usr/bin/original-awk '{ print $3 }')}"
...
get_mem_usage_osx(){
  ...
  vm_stat | /usr/bin/original-awk -v page_size=$page_size -F ':' '
    BEGIN { free=0; used=0 }
...
get_mem_usage_freebsd(){
  vmstat -H | tail -n 1 | /usr/bin/original-awk '{ print $5, $4 }'
}
