...
print_cpu_usage() {
  ...
  cpu_view="${cpu_view//'#{cpu.color}'/$(echo "$cpu_color" | /usr/bin/original-awk '{ print $1 }')}"
  cpu_view="${cpu_view//'#{cpu.color2}'/$(echo "$cpu_color" | /usr/bin/original-awk '{ print $2 }')}"
  cpu_view="${cpu_view//'#{cpu.color3}'/$(echo "$cpu_color" | /usr/bin/original-awk '{ print $3 }')}"
